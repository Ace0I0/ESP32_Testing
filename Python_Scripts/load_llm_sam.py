...
def build_system_prompt(identity: dict, mode: dict, user_text: str) -> tuple[str, dict]:
    ...
    if route.get("intent") != "general_eas_info" or route.get("topics"):
        prompt_prefix.extend([
            "",
            f"Detected intent: {route.get('intent', 'general_eas_info')}",
            f"Detected topics: {', '.join(route.get('topics', [])) if route.get('topics') else 'none'}",
        ])
    ...
    budgeted_knowledge_context = budget.enforce_prompt_budget(
        prompt_without_knowledge,
        knowledge_context,
        user_text,
    )

    # Final prompt assembly happens here, after the knowledge context has been
    # budgeted and trimmed.
    system_prompt = "\n".join(
        prompt_prefix
        + ["", "Relevant local knowledge:"]
        + ([budgeted_knowledge_context] if budgeted_knowledge_context else [])
        + prompt_suffix
    )

    estimated_final_prompt_tokens = budget.estimate_tokens(system_prompt) + budget.estimate_tokens(user_text)
    ...
    debug_info = {
        ...
        "max_knowledge_tokens": budget.MAX_KNOWLEDGE_TOKENS,
        "query_keywords": get_knowledge_query_keywords(user_text),
        "router": route,
        ...
        "budget_trimmed": budgeted_knowledge_context != knowledge_context,
    }
...
def print_prompt_debug(debug_info: dict) -> None:
    ...
    # Router and budget details are printed only in debug mode.
    route = debug_info.get("router")
    if route:
    ...
    debug_log(
        "knowledge retrieval: "
        f"budget={debug_info['knowledge_token_budget']} tokens, "
        f"query_keywords={', '.join(debug_info['query_keywords'])}"
    )
    debug_log("selected knowledge files:")

    for file_info in debug_info["selected_file_details"]:
        status = "loaded" if file_info.get("exists", True) else "missing"
        debug_log(
            f"  {file_info['file']} - {status}, "
            f"reason={file_info['reason']}, "
            f"allocated={file_info['allocated_context_tokens']}, "
            f"used={file_info['estimated_used_tokens']}, "
            f"trimmed={file_info['trimmed']}"
        )
    ...
    debug_log(
        ...
        f"estimated_final={debug_info['estimated_final_prompt_tokens']} tokens"
    )
...
def print_server_debug(server_debug: dict) -> None:
    ...
    debug_log(
        "request settings: "
        f"response_tokens={server_debug.get('response_tokens')}, "
...
def main() -> None:
    ...
    while True:
        ...
        try:
            ...
            robot_text, server_debug = ask_local_model(user_text, system_prompt, prompt_debug["response_tokens"])
